...
pub fn Chart(pair: ReadSignal<String>) -> impl IntoView {
    let candles = create_resource(move || pair.get(), |p| async move {
        match api::get(&format!("/api/candles/{}?interval=1h&limit=60", p)).await {
        ...
        <div class="chart-wrapper">
            <Suspense fallback=move || view! {
                ...
                {move || candles.get().map(|data| {
                    ...
                    let path_d = closes.iter().enumerate().map(|(i, &c)| {
